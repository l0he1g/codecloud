Ext.onReady(function(){
	...
	var daydataUrl="../../controller/processHost.php?operation=1";	//线状图表统计
	var urlUrl="../../controller/processHost.php?operation=4";		//url数据
	var useridUrl="../../controller/processHost.php?operation=5";	//userid数据
	var upStreamUrl="../../controller/processHost.php?operation=6";	//up-stream统计数据
	var downStreamUrl="../../controller/processHost.php?operation=7";	//down-stream统计数据
	...
	var dataLoad=function(store,records,opts){
		...
		switch(fieldName[0]){
			...
			case 'upstream':
				params={
					gridKeys:['upstream','num']
					,chartKeys:['upstream','num']
					,title:'up-stream'
					,xtype:Ext.ux.PiechartGrid
					,keyword:'upstream'
				}
			...
			case 'downstream':
				params={
					gridKeys:['downstream','num']
					,chartKeys:['downstream','num']
					,title:'down-stream'
					,xtype:Ext.ux.PiechartGrid
					,keyword:'downstream'
				}
		...
		,listeners:{
			load:function(store,records,opts){
				store.each(function(record){
					var dt = new Date();
					dt = Date.parseDate(record.get('month'),"Ym");
					var nameStr=dt.format('Y年m月');
...
//组装面板
	...
	var centerPanel=new Ext.Panel({
		...
		,tbar:new Ext.ux.dateToolbar({
			queryStore:[dataStore2,dataStore3,dataStore4,dataStore5,dataStore6]
